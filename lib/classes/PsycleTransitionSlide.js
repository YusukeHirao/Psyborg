...
PsycleTransition_1.default.create({
    slide: {
        init: function (psycle) {
            // スタイルを設定
            StyleSheet_1.default.posBase(psycle.stage.$el);
            StyleSheet_1.default.posAbs(psycle.container.$el);
            StyleSheet_1.default.posBase(psycle.panels.$el);
            StyleSheet_1.default.floating(psycle.panels.$el);
            // 初期のスタイルを保存
            StyleSheet_1.default.saveCSS(psycle.panels.$el);
            // 初期化時のインラインスタイルを保持
        ...
        reflow: function (psycle, info) {
            switch (info.timing) {
                case PsycleReflowTiming_1.default.TRANSITION_END: {
                    var distination = psycle.panelWidth * psycle.index * -1 + (psycle.cloneCount * psycle.panelWidth * psycle.length * -1);
                ...
                case PsycleReflowTiming_1.default.LOAD: {
                    ...
                    var $panels = psycle.panels.$el;
                    var $container = psycle.container.$el;
                    /**
                    ...
                    // コンテナの幅を計算
                    var containerWidth = panelOuterWidth * psycle.length;
                    // ループの時の処理
                    if (psycle.repeat === PsycleRepeat_1.default.LOOP) {
                        /*
                            * ステージがコンテナに対して何倍大きいか
                            *
                            ...
                            *
                            */
                        var stageWidthRatio = psycle.stageWidth / containerWidth;
        ...
        fire: function (psycle) {
            ...
            var duration = psycle.duration || psycle.config.duration;
            var distination = psycle.panelWidth * (psycle.index + psycle.vector) * -1 + (psycle.cloneCount * psycle.panelWidth * psycle.length * -1);
