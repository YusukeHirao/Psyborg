module psyborg {
	...
	export class Psycle extends PsyborgElement {
		...
		constructor ($el:JQuery, options: IPsycleConfig) {
			super($el);
			this.config = <IPsycleConfig> $.extend(<IPsycleConfig> {
				instanceKey: 'psycle',
				startIndex: 0,
				transition: 'slide',
				duration: 600,
		...
		public transitionTo (to: number, duration?: number, direction: number = 0, vector?: number, fromHalfway: boolean = false): Psycle {
			...
			let optimizedVector: number = $.isNumeric(vector) ? vector : this._optimizeVector(to, direction);
		...
		private _resizeable (): void {
			const resizeEndDelay: number = 300;
